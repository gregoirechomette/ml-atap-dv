...
class Dataset:
    ...
    def prepare_data_population(self, minLat, maxLat, minLon, maxLon):
        
        # Retrieve the data
        data = pd.read_csv(self.csv_name, usecols=self.inputs+self.outputs)
        data = data[(data['latitude'] > minLat) & (data['latitude'] < maxLat) & (data['longitude'] > minLon) & (data['longitude'] < maxLon)]
        ...
        # Put output in different categories
        data_outputs[self.outputs[0]].mask(data_outputs[self.outputs[0]] > 0, 1, inplace=True)
        self.y_class = data_outputs.iloc[:, :].values
